...
describe('Shape Classes', () => {
  test('Triangle render method should return SVG string with correct color', async () => {
    ...
    expect(svg).toContain(`<polygon points="50,10 10,190 190,190" style="fill:${color}"/>`);
    expect(svg).toContain(`<text x="100" y="150" text-anchor="middle" alignment-baseline="middle" fill="black">${text}</text>`);
  });
  ...
  test('Square render method should return SVG string with correct color', async () => {
    ...
    expect(svg).toContain(`<text x="150" y="50" text-anchor="middle" alignment-baseline="middle" fill="black">${text}</text>`);
  });
});
